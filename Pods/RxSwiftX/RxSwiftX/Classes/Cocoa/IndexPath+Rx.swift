...
public extension Driver where E == IndexPath {
    ...
    func section(_ section: Int) -> SharedSequence<SharingStrategy, E> {
        return filter { $0.section == section }
    }
    
    func row(_ row: Int) -> SharedSequence<SharingStrategy, E> {
        return filter { $0.row == row }
    }
    
    func item(_ item: Int) -> SharedSequence<SharingStrategy, E> {
        return filter { $0.item == item }
    }
}
